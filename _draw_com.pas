...
begin
   ...
   begin
      ...
      begin
         ...
         case c of
         #0..#4  : begin cl:=plcolor[ord(c)];if(i<ss)then continue;end; //tc:=cl;
         #11..#13: ;
         ...
         #25     : begin cl:=tc             ;if(i<ss)then continue;end;
           ...
           case c of
             hp_char   : boxColor(sur,ix,y,ix+font_iw,y+font_iw,c_orange);
...
procedure ui_addalrm(ax,ay:integer;ab:boolean);
var i,ni:byte;
begin
   if(_ded)or(_rpls_rst>=rpl_rhead)then exit;

   ni:=255;
   for i:=0 to vid_uialrm_n do
    with ui_alrms[i] do
     if(t>0)then
      if(dist2(ax,ay,x,y)<=vid_uialrm_mr)and(b=ab)then
      begin
         x:=(x+ax) div 2;
         y:=(y+ay) div 2;
         if(t<vid_uialrm_ti)then t:=vid_uialrm_t;
         ni:=i;
         break;
      end;
    ...
    for i:=0 to vid_uialrm_n do
     with ui_alrms[i] do
      ...
      begin
         ...
         b:=ab;
         t:=vid_uialrm_t;
         if((vid_mmvx-vid_uialrm_ti)>x)or(x>(vid_mmvx+map_mmvw+vid_uialrm_ti))or   // vid_mmvx,vid_mmvy,vid_mmvx+map_mmvw,vid_mmvy+map_mmvh
           ((vid_mmvy-vid_uialrm_ti)>y)or(y>(vid_mmvy+map_mmvh+vid_uialrm_ti))then PlaySND(snd_alarm,0);
         break;
      end;
end;

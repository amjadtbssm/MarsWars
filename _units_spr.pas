...
begin
   ...
   an:=td;
   if(rld>rld_a)or(buff[ub_pain]>0)then an:=td+8;

   _unit_spr:=@spr_octo[an];
end;
UID_Cyclope :
begin
   td:=((dir+23) mod 360) div 45;
   ...
   begin
      if(wanim)then inc(anim,anims);
      anim:=(anim mod 400);
      an:=td+8*(anim div 100);
   end;

   _unit_spr:=@spr_cycl[an];
end;
...
UID_Octobrain:begin td:=16+abs(hits div 10);if(td>19)then td:=19;_unit_spr:=@spr_octo[td];end;//16-19
...
UID_Cyclope    : begin td:=48+abs(hits div 25);if(td>54)then td:=54;_unit_spr:=@spr_Cycl       [td];end; // 48-54
